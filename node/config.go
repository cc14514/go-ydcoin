...
import (
	...
	"github.com/cc14514/go-ydcoin/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of geth is "ydcoin". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "ydcoin", keep that.
	if name == "ydcoin" || name == "ydcoin-testnet" {
		name = "ydcoin"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "ydcoin" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "ydcoin" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "ydcoin" {
...
func (c *Config) loadECDSAWithPwd() (*ecdsa.PrivateKey, error) {
	...
	overFunc := func(){
		fmt.Println("=================================================")
		fmt.Println(`> Must be first execute "ydcoin security --unlock" `)
		fmt.Println("=================================================")
		os.Exit(-1)
	}
